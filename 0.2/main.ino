...
void setup() {
  ...
  //turn the PID on
  PID_pos_l.SetMode(AUTOMATIC);
  PID_pos_r.SetMode(AUTOMATIC);
  //PID_pos_l.SetSampleTime(20);
  //PID_pos_r.SetSampleTime(20);
  //
...
void loop() {
 ...
 //while (!mpuInterrupt && fifoCount < packetSize) {
  ...
  if ((mpuIntStatus & 0x10) || fifoCount == 1024) {
    ...
    //Serial.print("ypr\t");
    if(ypr[2] !=0){Serial.print(ypr[2] * 180 / M_PI);
    Serial.print(",");
    //Serial.print("\t");
  ...
  //Serial.println(en_l_pos_a);
  //Serial.print(",");
  //Serial.println(en_r_pos_a);

  
  Input_pos_l = ypr[2] * 180 / M_PI;
  Input_pos_r = ypr[2] * 180 / M_PI;
  PID_pos_l.Compute();
  PID_pos_r.Compute();
  //
  ...
  motor_work(motor_r, Output_pos_l);

}
Serial.println(Output_pos_l);
}
